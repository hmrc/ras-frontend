...
class TaxYearResolverSpec extends AnyWordSpec with Matchers {
  ...
  object Resolver {
    ...
    def apply(currentTime: DateTime): TaxYearResolver = new TaxYearResolver {
      override lazy val now: () => DateTime = () => currentTime
    }

    def apply(): TaxYearResolver = new TaxYearResolver {
      override lazy val now: () => DateTime = () => new DateTime(2013, 9, 24, 11, 39, 55, 222, DateTimeZone.forID("Europe/London"))
    }
  }
  ...
  "local date falling in this tax year" must {
    ...
    "return true when earliestDateForCurrentTaxYear is the same as the current date" in {
      val taxYearStart = new LocalDate(TaxYearResolverForTest.taxYearFor(TaxYearResolverForTest.now().toLocalDate), 4, 6)
      TaxYearResolverForTest.fallsInThisTaxYear(taxYearStart) shouldBe true
    }
  }
