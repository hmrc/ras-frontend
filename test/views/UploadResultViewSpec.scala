...
class UploadResultViewSpec extends ViewSpecHelper {
  ...
  "upload result page" must {
    behave like pageWithFeedbackLink(
      uploadResultView("fileId", formattedExpiryDate(now), isBeforeApr6(now), currentTaxYear = 1000, "filename")(
        fakeRequest,
        testMessages,
        mockAppConfig
      )
    ...
    "contain a result link pointing to the results file" in {
      val result =
        uploadResultView("testFileId", formattedExpiryDate(now), isBeforeApr6(now), currentTaxYear = 1000, "filename")(
          fakeRequest,
          testMessages,
          mockAppConfig
        )
    ...
    "contain expiry date message" in {
      ...
      doc(result).getElementById("expiry-date-message").text shouldBe Messages(
        "expiry.date.message",
        formattedExpiryDate(now)
      )
    }
    ...
    "contain what to do next content" in {
      ...
      doc(result).getElementById("whatnext-content").text shouldBe Messages(
        "upload.result.what.next",
        Messages("upload.result.member.contact")
      )
    }
    ...
    "contains an HMRC link that opens help page in new tab" in {
      ...
      doc(result)
        .getElementById("contact-hmrc-link")
        .attr(
          "href"
        )                                                            shouldBe "https://www.gov.uk/government/organisations/hm-revenue-customs/contact/national-insurance-numbers"
    ...
    "contain the correct ga events when upload date is 01/01/2018 (CY+1)" in {
      ...
      val result              = uploadResultView(
        "fileId",
        formattedExpiryDate(mockUploadTimeStamp),
        isBeforeApr6(mockUploadTimeStamp),
        currentTaxYear = 1000,
        "filename"
      )(fakeRequest, testMessages, mockAppConfig)
      doc(result)
        .getElementById("back")
        .attr("data-journey-click")                                        shouldBe "navigation - link:Residency status upload added CY & CY + 1:Back"
      doc(result)
        .getElementById("result-link")
        .attr(
          "data-journey-click"
        )                                                                  shouldBe "link - click:Residency status upload added CY & CY + 1:ResidencyStatusResults CY & CY + 1 CSV"
      doc(result)
        .getElementById("look-up-another-member-link")
        .attr("data-journey-click")                                        shouldBe "link - click:User details not found:Look up another member"
      doc(result).getElementById("upload-file").attr("data-journey-click") shouldBe "look-up-multiple"
      doc(result)
        .getElementById("contact-hmrc-link")
        .attr(
          "data-journey-click"
        )                                                                  shouldBe "link - click:Residency status upload added CY & CY + 1:Member must contact HMRC"
    }
    ...
    "contain a cy message when upload date is 06/04/2018" in {
      ...
      doc(result).getElementById("cy-message").text shouldBe Messages(
        "cy.message",
        (1000 + 1).toString,
        (1000 + 2).toString
      )
    ...
    "contain the correct ga events when upload date is 06/04/2018 (CY only)" in {
      ...
      doc(result)
        .getElementById("back")
        .attr("data-journey-click") shouldBe "navigation - link:Residency status upload added CY:Back"
      doc(result)
        .getElementById("result-link")
        .attr(
          "data-journey-click"
        )                           shouldBe "link - click:Residency status upload added CY:ResidencyStatusResults CY CSV"
      doc(result)
        .getElementById("look-up-another-member-link")
        .attr("data-journey-click") shouldBe "link - click:User details not found:Look up another member"
      doc(result)
      ...
      doc(result)
        .getElementById("contact-hmrc-link")
        .attr("data-journey-click") shouldBe "link - click:Residency status upload added CY:Member must contact HMRC"
