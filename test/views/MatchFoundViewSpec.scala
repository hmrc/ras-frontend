...
class MatchFoundViewSpec extends ViewSpecHelper {
  ...
  "match found page" must {
    ...
    behave like pageWithFeedbackLink(
      matchFoundView("Jim Mcgill", dobFormatted, "AA123456A", NON_SCOTTISH, Some(SCOTTISH), 1000, 1001)(
        fakeRequest,
        testMessages,
        mockAppConfig
      )
    ...
    "contain correct title when match found" in {
      val result = matchFoundView("Jim Mcgill", dobFormatted, "AA123456A", NON_SCOTTISH, Some(SCOTTISH), 1000, 1001)(
        fakeRequest,
        testMessages,
        mockAppConfig
      )
    ...
    "contain customer details and residency status when match found and CY and CY+1 is present" in {
      val result = matchFoundView("Jim Mcgill", dobFormatted, "AA123456A", SCOTTISH, Some(NON_SCOTTISH), 1000, 1001)(
        fakeRequest,
        testMessages,
        mockAppConfig
      )
    ...
    "contain correct ga events when match found and CY and CY+1 is present" in {
      ...
      doc(result)
        ...
        .attr("data-journey-click") shouldBe "link - click:Residency status added CY & CY + 1:Look up another member"
    }

    "contain customer details and residency status when match found and only CY is present" in {
      val result = matchFoundView("Jim Mcgill", dobFormatted, "AA123456A", SCOTTISH, None, 1000, 1001)(
        fakeRequest,
        testMessages,
        mockAppConfig
      )
      doc(result).getElementById("what-happens-next-content").text    shouldBe Messages("match.found.top")
      doc(result).getElementById("what-happens-next-sub-header").text shouldBe Messages("match.found.what.happens.next")
      doc(result).getElementById("bottom-content-cy").text            shouldBe Messages(
        "match.found.bottom.current-year.bottom",
        (1000 + 1).toString,
        "Jim Mcgill",
        (1000 + 1).toString,
        (1000 + 2).toString
    ...
    "contain correct ga event when match found and only CY is present" in {
      val result = matchFoundView("Jim Mcgill", dobFormatted, "AA123456A", NON_SCOTTISH, None, 1000, 1001)(
        fakeRequest,
        testMessages,
        mockAppConfig
      )
      ...
      doc(result)
        .getElementById("look-up-another-member-link")
        .attr("data-journey-click") shouldBe "link - click:Residency status added CY:Look up another member"
    }

    "display correct residency status for UK UK" in {
      val result =
        matchFoundView("Jim Mcgill", dobFormatted, "AA123456A", NON_SCOTTISH, Some(NON_SCOTTISH), 1000, 1001)(
          fakeRequest,
          testMessages,
          mockAppConfig
        )
    ...
    "contain a look up another member link when match found" in {
      ...
      doc(result)
        .getElementById("look-up-another-member-link")
        .attr("href") shouldBe "/relief-at-source/check-another-member/member-name?cleanSession=true"
    }
  }
