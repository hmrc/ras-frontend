...
class MatchNotFoundViewSpec extends ViewSpecHelper {
  ...
  "match not found page" must {
    ...
    behave like pageWithFeedbackLink(
      matchNotFoundView("Jim McGill", dob, nino)(fakeRequest, testMessages, mockAppConfig)
    )
    ...
    "contain what to do next section when match not found" in {
      val result = matchNotFoundView("Jim McGill", dob, nino)(fakeRequest, testMessages, mockAppConfig)
      doc(result).getElementById("what-to-do").text shouldBe Messages("match.not.found.what.to.do") + " " + Messages(
        "contact.hmrc",
        "Jim McGill"
      )
    }
    ...
    "contain ga event data when match not found " in {
      ...
      doc(result)
        ...
        .attr("data-journey-click") shouldBe "navigation - link:Users details not found:Back"
      doc(result)
        .getElementById("change-name-link")
        .attr("data-journey-click") shouldBe "link - click:User details not found:Change Name"
      doc(result)
        .getElementById("change-nino-link")
        .attr("data-journey-click") shouldBe "link - click:User details not found:Change NINO"
      doc(result)
        .getElementById("change-dob-link")
        .attr("data-journey-click") shouldBe "link - click:User details not found:Change DOB"
      doc(result)
        .getElementById("look-up-another-member-link")
