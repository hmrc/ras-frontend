...
import helpers.{I18nHelper, RandomNino}
...
class MemberNinoFormSpec extends UnitSpec with I18nHelper with OneAppPerSuite {
  ...
  "Find member details form" should {
    ...
    "return an error when nino field is empty" in {
      ...
      assert(validatedForm.errors.contains(FormError("nino", List(Messages("error.membername.mandatory", "James Potter", Messages("nino"))))))
    }
