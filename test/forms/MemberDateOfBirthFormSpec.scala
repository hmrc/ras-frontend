...
class MemberDateOfBirthFormSpec extends AnyWordSpec with RasTestHelper with DateValidator {
  ...
  "DateValidator.updatedErrors" when {
    "dobIsEmpty should return FormError against dateOfBirth with key error.dob.missing" in {
      val formWithErrors        =
        form().bind(Map("dateOfBirth.day" -> "", "dateOfBirth.month" -> "", "dateOfBirth.year" -> ""))
    ...
    "dobDayEmpty should return FormError against dateOfBirth.day and dateOfBirth.month with key error.dob.missing.day.month" in {
      val formWithErrors        =
        form().bind(Map("dateOfBirth.day" -> "", "dateOfBirth.month" -> "", "dateOfBirth.year" -> "1999"))
      val updatedFormWithErrors = formWithErrors.copy(errors =
        Seq(
    ...
    "dobMonthEmpty should return FormError against dateOfBirth.month with key error.dob.missing.month.year" in {
      val formWithErrors        =
        form().bind(Map("dateOfBirth.day" -> "1", "dateOfBirth.month" -> "", "dateOfBirth.year" -> ""))
      val updatedFormWithErrors = formWithErrors.copy(errors =
        Seq(
    ...
    "dobYearEmpty should return FormError against dateOfBirth.year with key error.dob.missing.day.year" in {
      val formWithErrors        =
        form().bind(Map("dateOfBirth.day" -> "", "dateOfBirth.month" -> "3", "dateOfBirth.year" -> ""))
      val updatedFormWithErrors = formWithErrors.copy(errors =
        Seq(
